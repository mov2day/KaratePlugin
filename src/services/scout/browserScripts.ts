/** Shared, dependency-free browser instrumentation used by both connection adapters. */
export const INSTALL_CAPTURE = String.raw`(config) => {
...
export const PERFORM_ACTION = String.raw`async (action) => {
...
export function captureExpression(config: { enabled: boolean; allowedOrigins: string[]; excludedOrigins: string[] }): string {
    return `(${INSTALL_CAPTURE})(${JSON.stringify(config)})`;
}

export function actionExpression(action: unknown): string {
