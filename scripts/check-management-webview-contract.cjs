...
const requirements = [
  ...
  ['disposed webview delivery isolation', /Promise\.resolve\(webview\.postMessage\(message\)\)\.catch\(\(\) => this\.removeManagementWebview\(webview\)\)/],
  ['disposed sidebar reference cleanup', /if \(this\._viewWebview === webview\)/],
  ['disposed expanded reference cleanup', /if \(this\._expandedWebview === webview\)/],
  ['disposed sidebar callback avoids disposed view access', /webviewView\.onDidDispose\(\(\) => this\.removeManagementWebview\(webview\)\)/],
  ['disposed expanded callback avoids disposed panel access', /panel\.onDidDispose\(\(\) => \{\s*this\.removeManagementWebview\(webview\)/],
  ['per-webview readiness', /_readyManagementWebviews/],
  ['existing expanded workspace reveal', /existing\.reveal\(existing\.viewColumn \|\| vscode\.ViewColumn\.Active, false\)/],
  ['global open command uses expanded workspace', /'karate-dsl\.openPanel',[\s\S]*webviewProvider\.openExpandedWorkspace\(\)/],
  ['expanded workspace state synchronization', /type: 'expandedWorkspaceState'/],
  ['management area synchronization', /managementAreaChanged/],
  ['compact full-view cue', /layout-toggle \$\{expandNudge \? 'is-nudged' : ''\}/],
  ['existing expanded workspace show label', /expandedOpen \? 'Show' : 'Open'/],
